...
const JWT =
  "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..i-UnCaUnIoKd_S19krFwqOxEKUcAA7XWfrtUEfn2VEA";

// const BASE_URL = "https://launch-pad-g69q.vercel.app/api";

//const BASE_URL = "launchpad-wine-three.vercel.app/api";
const BASE_URL = "https://launchpad-sage-sigma.vercel.app/api";
...
export const postLaunch = async (data) => {
  ...
  try {
    ...
    const response = await axios.post(
      `${BASE_URL}/launchSetting/upsertLaunchSettings`,
      data
    );
...
export const upsertWallet = async (data, id) => {
  ...
  try {
    const response = await axios.post(
      `${BASE_URL}/walletService/upsertWallet`,
      {
        wallets: data,
        projectId: id,
      }
...
export const TokenLaunch = async (data) => {
  ...
  try {
    // data.userId = localStorage.getItem("userId");
    const response = await axios.post(
      `${BASE_URL}/projectSettingsService/getProjectDetail`,
      {
...
export const mintToken = async (data) => {
  ...
  try {
    // data.userId = localStorage.getItem("userId");
    const response = await axios.post(
      `${BASE_URL}/mintSettingsService/upsertProject`,
      {
...
export const getMintsettings = async (data) => {
  ...
  try {
    const response = await axios.post(
      `${BASE_URL}/mintSettingsService/getProject`,
      {
        userId: localStorage.getItem("userId"),
      }
    );
